...
function isCoverOrProfile(post: RawPost): boolean {
  for (const att of post.attachments?.data ?? []) {
    ...
    if (
      type.includes("cover_photo") ||
      type.includes("profile_media") ||
      type.includes("profile_photo")
    ) {
      return true;
    }
  }
  const status = (post.status_type ?? "").toLowerCase();
  if (status.includes("cover_photo") || status.includes("profile_media")) return true;
  const story = (post.story ?? "").toLowerCase();
  return (
    story.includes("foto da capa") ||
    story.includes("foto de capa") ||
    story.includes("cover photo") ||
    story.includes("foto do perfil") ||
    story.includes("foto de perfil") ||
...
export async function fetchPagePosts(options: {
  ...
  const { pageId, token, since, until, maxPages = 200 } = options;
...
/** Exclusão em lote via batch API, com fallback individual. */
  ...
  try {
    ...
    return postIds.map((id, index) => {
      ...
      if (entry && entry.code === 200) {
        try {
          const parsed = JSON.parse(entry.body ?? "{}") as { success?: boolean } & GraphError;
          if (parsed.success || !parsed.error) return { id, ok: true };
          return {
            id,
            ok: false,
            error: parsed.error?.message ?? "Não confirmou exclusão.",
          };
        } catch {
          return { id, ok: true };
        }
